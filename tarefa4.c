...
/* Descrição: Tarefa 4 referente ao TP0 da disciplina. O objetivo é   */
/* fazer com que cada proxcesso execute testes até encontrar o        */
/* primeiro processo correto e atualize seu vetor State[N],           */
/* realizando a comparação com o vetor State[N] do processo testado.  */
/*                                                                    */
/**********************************************************************/
...
void log_header(int n) {
  printf("**********************************************************\n");
  printf("*                Início do Log - tarefa4.c               *\n");
  printf("**********************************************************\n");
...
/* update_state(current_process_state, next_process_state)
   current_process_state: vetor de estados do processo que está realizado o
   teste
   next_process_state: vetor de estados do processo que está sendo testado
   next: processo que será comparado no vetor de estados
   token: id do processo testador
   n: número de processos
 */
int *update_state(int *current_process_state, int *next_process_state, int next,
                  int token, int n) {
  do {
    next = (next + 1) % n;
    if (next_process_state[next] > current_process_state[next]) {
      current_process_state[next] = next_process_state[next];
    }
  } while (next != token);
        ...
        for (i = 1; i <= N; i++) {
          ...
          if (status(processo[next].id) != 0) {
            printf("%4.1f\t[TEST]  Processo %d -> Processo %d: FALHA\n", time(),
                   token, next);
            // Se o status do processo era desconhecido e ele falha, vai direto
            // de -1 para 1
            if (processo[token].state[next] == -1) {
              processo[token].state[next] = 1;
            } else {
              // Caso contrario, apenas incrementa
            ...
            processo[token].state = update_state(
                processo[token].state, processo[next].state, next, token, N);
            break;
          }
        }
